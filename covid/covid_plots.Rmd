...
if (get_dhhs_url(today[2],today[1])!=FALSE){
  load("covid_data.rda")
  # if there is no today.raw in calculated dates, then update
  if (!any(covid_data$date == today.raw)){
    ...
    covid_data <- cbind(covid_data,update.covid)
    save(covid_data, file = "covid_data.rda")
  }
}
```
