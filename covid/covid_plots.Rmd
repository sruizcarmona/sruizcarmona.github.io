...
fig <- fig %>% add_trace(y = ~active, name = 'Active', mode = 'lines',line = list(color = mycols[2]))
fig <- fig %>% add_trace(y = ~recovered, name = 'Recovered', mode = 'lines',line = list(color = mycols[3]))
...
fig <- fig %>% add_trace(y = ~deaths, name = 'Deaths', mode = 'lines',yaxis = 'y2',line = list(color = mycols[6]))
fig <- fig %>% layout(title = '',
                      xaxis = list(title = "Date",tickangle=-60, type='date', tickformat= '%b %e',
                                   range = ~date, autotick=F),
                      yaxis = list(side = 'left', title = 'Total / Active / Recovered Cases (#)', range=c(0,1500)),
                      yaxis2 = list(side = 'right', overlaying = "y", title = 'Hospital / ICU \n Deaths / Community (# people)',
                                    showgrid=FALSE, zeroline=F, range=c(0,150), textangle = 90),
                      margin = list(r = 70),
                      legend = list(x = 0.05, y = 0.95, bordercolor = "black", borderwidth=0.5),
                      hovermode='x')
fig
```
```{r}
