...
get_data_from_url <- function(day,month,item){
  ...
  if (item == 'patients'){
    ...
    pat_clean <- pat_clean[!str_detect(pat_clean,"community")]
    pat_clean <- pat_clean[!str_detect(pat_clean,"tests")]
    # split by words
...
fig <- plot_ly(data = covid_data,x = ~date)
...
fig <- fig %>% add_trace(y = ~active, name = 'Active',type='scatter', mode = 'lines',line = list(color = mycols[3]))
...
fig <- fig %>% layout(title = '',
                      xaxis = list(title = "Date",tickangle=-60, type='date', tickformat= '%b %e',
                                   range = ~date, autotick=F, domain=c(0,0.92),
                      ...
                      yaxis2 = list(side = 'right', overlaying = "y", anchor='x',
                                    title = 'Daily Cases / Hospital / ICU \n Deaths / Community (# people)',
                                    linewidth = 3,
                                    showgrid=FALSE, zeroline=T, range=c(0,180), textangle = 90),
                      yaxis3 = list(side = 'right', overlaying = "y",
                                    ...
                                    linewidth = 3,
                                    showgrid=FALSE, zeroline=T, range=c(0,6), textangle = 90),
                      margin = list(r = 25),
